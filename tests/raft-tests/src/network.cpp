...
{
  public:
    MOCK_METHOD((tl::expected<raft::data::AppendEntriesResponse, raft::Error>),
                handleAppendEntries,
                (const raft::data::AppendEntriesRequest& request),
                (override));

    MOCK_METHOD((tl::expected<raft::data::RequestVoteResponse, raft::Error>),
                handleRequestVote,
...
TEST(NetworkTest, ValidAppendEntriesAnyPort)
{
    ...
    EXPECT_CALL(*mockHandler, handleAppendEntries(request))
        .Times(1)
...
TEST(NetworkTest, ValidRequestVoteAnyPort)
{
    ...
    EXPECT_CALL(*mockHandler, handleRequestVote(request))
        .Times(1)
        .WillOnce(testing::Return(response));
...
TEST(NetworkTest, ValidAppendEntriesGivenPort)
{
    ...
    // Verify that the provided address and returned address match
...
TEST(NetworkTest, AppendEntriesInvalidArgumentResponse)
{
    ...
    EXPECT_CALL(*mockHandler, handleAppendEntries(request))
        .Times(1)
        .WillOnce(testing::Return(tl::unexpected(error)));
